...
let selectedServer = 'ghproxyOrg'

let proxyMap = {
  // https://ghproxy.org/
  ghproxyOrg: {
    down: "https://ghproxy.org/https://github.com/",
    raw: "https://ghproxy.org/https://raw.githubusercontent.com/",
    home: "https://ghproxy.org/https://github.com/"
  },
...
let include = [
  {
    ...
    , to: (url) => url.replace("https://github.com/", proxyMap[selectedServer].down)
  }
  , {
    match: (url) => url.startsWith("https://raw.githubusercontent.com/")
    , to: (url) => url.replace("https://raw.githubusercontent.com/", proxyMap[selectedServer].raw)
  }
  , {
    match: (url) => url.startsWith("https://github.com/")
    , to: (url) => url.replace("https://github.com/", proxyMap[selectedServer].home)
  }
]
...
module.exports = class ProxyGithub extends Plugin {
  onload() {
    ...
    apc.regedit();
    app.regedit();
    this.settings = { server: selectedServer }
    this.loadSettings();
  }
  async loadSettings() {
    this.settings = Object.assign({}, { server: selectedServer }, await this.loadData());
  }
  async saveSettings() {
    await this.saveData(this.settings);
    selectedServer = this.settings.server;
    debugger
  }
